...
int hists_15750 () {
  ...
  TFile *hist_file = TFile::Open(Form("./all_runs/qa_output_nopedestal_00016945/histograms.root"));
  assert(hist_file);

  char *file_prefix = strdup("./hists_16945");
  ...
  for (int i = 0; i < 384; i++) {
    ...
    */
    ...
    delete h_alladc[i]->GetListOfFunctions()->FindObject("functionName");
